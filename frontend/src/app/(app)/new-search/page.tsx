...
import {
  Building2, FileSpreadsheet, Loader2, PlayCircle, UploadCloud,
} from "lucide-react";
...
export default function NewSearchPage() {
  ...
  async function runSingle(e: React.FormEvent) {
    ...
    try {
      const searches = await api.startSearch([{
        name, website, location, linkedin_url: linkedin, industry,
      }]);
  ...
  async function runBulk(e: React.FormEvent) {
    ...
    const companies = bulk
      ...
      .map((line) => {
        ...
        return { name: parts[0] || "", website: parts[1] || "", location: parts[2] || "" };
      })
      .filter((c) => c.name && c.website);
    if (companies.length === 0) {
      setError("Enter at least one line like: Company Name, https://company.com[, Location]");
      return;
    }
    setBusy(true);
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not start searches.");
      setBusy(false);
    }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">New search</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          The platform will crawl the official website first ({`contact / careers / team / people / HR pages`}),
          extract real emails only, classify HR context, and verify before showing results.
        </p>
      </div>
      ...
      <div className="flex space-x-2">
        {(["single", "upload"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "single" ? "Single company / bulk paste" : "Excel / CSV upload"}
          </button>
        ))}
      </div>
      ...
      {tab === "single" ? (
        <div className="space-y-6">
          <form onSubmit={runSingle}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Website *</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">LinkedIn URL</label>
                <input value={linkedin} onChange={(e) => setLinkedin(e.target.value)}
                  className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="https://linkedin.com/company/acme" />
              </div>
              <div className="sm:col-span-2">
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Industry</label>
          ...
          <form onSubmit={runBulk}
            ...
            <div className="text-sm font-bold text-slate-800 dark:text-slate-100">Bulk paste (up to 50)</div>
            <p className="text-xs text-slate-400">
              One per line: <code className="text-blue-500">Company Name, https://website.com, Location</code>
            </p>
            <textarea rows={5} value={bulk} onChange={(e) => setBulk(e.target.value)}
              ...
              placeholder={"Acme Software, https://acme-software.com, Ahmedabad\nBeta Consulting, https://beta.io, Berlin"} />
            <button type="submit" disabled={busy}
        ...
        <form onSubmit={runUpload}
          ...
          <p className="text-xs text-slate-400">
            Required columns: <code className="text-blue-500">Company Name</code> and{" "}
            <code className="text-blue-500">Website</code>. Optional: Location, LinkedIn URL, Industry.
          </p>
      ...
      <p className="text-xs text-slate-400">
        ...
        <Link href="/searches" className="text-blue-500 font-semibold hover:underline">Searches</Link>.
      </p>
    </div>
  );
}
