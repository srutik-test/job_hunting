...
import { X, Globe, Mail, User, ShieldCheck, Check, Layers, ExternalLink, Calendar, Info } from 'lucide-react';
      ...
      <div className="relative w-full max-w-4xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl overflow-hidden my-8">
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-xl border border-blue-500/20 bg-blue-50/30 dark:bg-blue-950/20">
            <div>
              <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">Status</span>
              <StatusBadge status={result.status} />
            </div>
            <div>
              <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">Confidence Score</span>
              <ConfidenceBadge score={result.confidence_score} />
            </div>
            <div>
              <span className="text-xs font-medium text-slate-500 dark:text-slate-400 block mb-1">Primary Public Source</span>
              <span className="text-xs font-mono text-slate-700 dark:text-slate-300 truncate block" title={result.source}>
                {result.source}
              </span>
            </div>
          </div>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 space-y-3 bg-slate-50/30 dark:bg-slate-800/30">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between items-center py-1 border-b border-slate-200/60 dark:border-slate-700/60">
                  ...
                  <span className="font-mono font-medium text-slate-900 dark:text-slate-100">{result.hr_email}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-200/60 dark:border-slate-700/60">
                  <span className="text-slate-500">Recruitment Email:</span>
                  <span className="font-mono font-medium text-slate-900 dark:text-slate-100">{result.recruitment_email}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-200/60 dark:border-slate-700/60">
                  <span className="text-slate-500">Careers Email:</span>
                  <span className="font-mono font-medium text-slate-900 dark:text-slate-100">{result.careers_email}</span>
                </div>
                <div className="flex justify-between items-center py-1">
                  <span className="text-slate-500">General Email:</span>
                  <span className="font-mono font-medium text-slate-900 dark:text-slate-100">{result.general_email}</span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 space-y-3 bg-slate-50/30 dark:bg-slate-800/30">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between items-center py-1 border-b border-slate-200/60 dark:border-slate-700/60">
                  ...
                  <span className="font-medium text-slate-900 dark:text-slate-100">{result.hr_name}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-200/60 dark:border-slate-700/60">
                  <span className="text-slate-500">Position / Job Title:</span>
                  <span className="font-medium text-slate-900 dark:text-slate-100">{result.hr_position}</span>
                </div>
                <div className="flex justify-between items-center py-1">
                  ...
                  {result.linkedin_profile !== 'Not Publicly Available' ? (
                    ...
                    >
                      <span className="truncate max-w-[200px]">{result.linkedin_profile}</span>
                ...
                {allEmails.map((item: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-50 dark:bg-slate-800 text-xs">
                    <div className="flex items-center space-x-2">
                      <span className="font-mono font-medium text-slate-800 dark:text-slate-200">{item.email}</span>
                    ...
                    <span className="text-slate-400 text-[11px] truncate max-w-[250px]" title={item.source_url}>
                      {item.source_url}
                    </span>
                  </div>
                ))}
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
              ...
              <div className="max-h-48 overflow-y-auto space-y-1.5 pr-2">
                {crawledPages.map((page: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between text-xs py-1 border-b border-slate-100 dark:border-slate-800/80">
                    <div className="flex items-center space-x-2 truncate max-w-[450px]">
                      ...
                      <a href={page.url} target="_blank" rel="noopener noreferrer" className="text-slate-700 dark:text-slate-300 hover:text-blue-600 truncate">
                        {page.url}
                      </a>
                    </div>
