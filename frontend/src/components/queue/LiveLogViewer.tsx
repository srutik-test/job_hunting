...
import { Terminal, Search, Trash2, ArrowDownCircle, CheckCircle2, ShieldAlert } from 'lucide-react';
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 px-4 py-3 bg-slate-900/80">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div className="flex items-center space-x-1.5 text-slate-400 font-semibold pl-2">
            ...
            <span className="text-[11px] uppercase tracking-wider text-slate-300">Live Crawl & Extraction Terminal</span>
      ...
      >
        {filteredLogs.length === 0 ? (
          ...
          filteredLogs.map((log, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start space-x-2.5 hover:bg-slate-900/60 px-1 py-0.5 rounded transition">
                <span className="text-slate-600 select-none shrink-0">{timeStr}</span>
                <span className={`font-bold shrink-0 text-[10px] ${badgeColor}`}>[{log.level}]</span>
