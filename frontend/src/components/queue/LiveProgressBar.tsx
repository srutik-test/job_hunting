...
export default function LiveProgressBar({ progress, onCancel }: LiveProgressBarProps) {
    ...
    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-xs space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          <div className={`flex h-10 w-10 items-center justify-center rounded-xl font-bold text-white shadow-lg ${
            ...
            {isCompleted ? <CheckCircle2 className="h-5 w-5" /> : isCancelled ? <XCircle className="h-5 w-5" /> : <Play className="h-5 w-5 fill-white" />}
          </div>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-xs font-semibold text-slate-700 dark:text-slate-300">
          <span>Processing {progress.processed_companies} of {progress.total_companies} Companies</span>
          <span>{pct}%</span>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
        ...
        <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-800 space-y-1">
          ...
          <p className="font-bold text-slate-900 dark:text-white">
            {isCompleted ? '0s (Finished)' : `${progress.estimated_remaining_seconds || 0}s`}
          </p>
        </div>
