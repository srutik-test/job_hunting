...
export default function PipelineDiagram() {
  const steps = [
    ...
    },
    {
      ...
      color: 'border-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400',
    },
    {
      num: 'Step 4',
      ...
      color: 'border-purple-500/30 bg-purple-50/50 dark:bg-purple-950/20 text-purple-600 dark:text-purple-400',
    },
    {
      num: 'Step 5',
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Multi-source discovery architecture ensuring 100% public, verifiable contact intelligence with zero guess logic.
          </p>
        </div>
      </div>
        ...
        {steps.map((s, idx) => {
          ...
          return (
            ...
            >
              <div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-wider opacity-70">{s.num}</span>
                  <Icon className="h-4 w-4" />
                </div>
                <h4 className="text-xs font-bold mt-1 text-slate-900 dark:text-slate-100">{s.title}</h4>
              </div>
              <p className="text-[11px] leading-relaxed text-slate-600 dark:text-slate-400">{s.desc}</p>
            </div>
          );
        })}
      </div>
