...
async def test_duplicate_registration_rejected(client):
    ...
    resp = await ac.post("/api/v1/auth/register", json={
        "name": "Dup", "email": "tester@example.com", "password": "AnotherPass1!",
    })
...
async def test_captcha_challenge_dev_math():
    ...
    try:
        ...
        a, b = body.split("+")
        answer = int(a) + int(b)
        ok, _ = await cap.verify_captcha(captcha_id=challenge["captcha_id"],
                                         captcha_answer=str(answer))
        assert ok
        # single-use: cannot reuse
        ok, reason = await cap.verify_captcha(captcha_id=challenge["captcha_id"],
                                              captcha_answer=str(answer))
        assert not ok
        # wrong answer rejected
        challenge2 = cap.issue_dev_challenge()
        ok, _ = await cap.verify_captcha(captcha_id=challenge2["captcha_id"],
                                         captcha_answer="-999")
        assert not ok
    finally:
...
async def test_password_reset_flow(client):
    ...
    resp = await ac.post("/api/v1/auth/reset-password", json={
        "token": token, "new_password": "N3wPassword!",
    })
    assert resp.status_code == 200

    # token is single use
    resp2 = await ac.post("/api/v1/auth/reset-password", json={
        "token": token, "new_password": "Other123!",
    })
    assert resp2.status_code == 400

    # login works with new password
    resp3 = await ac.post("/api/v1/auth/login", json={
        "email": "tester@example.com", "password": "N3wPassword!",
    })
