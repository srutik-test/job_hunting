...
def test_email_candidate_validation():
    ...
    assert (
        crawler._is_valid_email_candidate("hr@aspiresoftserv.com", "aspiresoftserv.com")
        is True
    )
    assert (
        crawler._is_valid_email_candidate("style@2x.png", "aspiresoftserv.com") is False
    )
    assert (
        crawler._is_valid_email_candidate("invalid-email", "aspiresoftserv.com")
        is False
    )
    assert (
        crawler._is_valid_email_candidate("test@example.com", "aspiresoftserv.com")
        is False
    )
