...
async def dashboard_stats(user: User = Depends(get_current_user),
    ...
    for status, field in (
        ...
        value = await count(
            select(func.count()).select_from(Search).where(
                Search.user_id == user.id, Search.status == status))
        setattr(stats, field, value)

    stats.total_contacts = await count(
        select(func.count()).select_from(HRContact).where(
            HRContact.user_id == user.id))
    stats.verified_contacts = await count(
        select(func.count()).select_from(HRContact).where(
            HRContact.user_id == user.id,
            HRContact.contact_category == "verified_hr"))
    ...
    stats.linkedin_profiles = await count(
        select(func.count()).select_from(HRContact).where(
            HRContact.user_id == user.id, HRContact.contact_category == "linkedin"))
    return stats
