...
class ExtractionCoordinator:
    ...
    async def process_company(
        ...
        add_job_log(
            job_id_str,
            "INFO",
            f"▶ Starting Public HR Contact Research for '{company_name}' ({website})",
        ...
        add_job_log(
            job_id_str,
            "INFO",
            f"Step 1: Crawling website structure for '{company_name}'...",
        )
        ...
        add_job_log(
            job_id_str,
            "INFO",
            f"Step 1 Completed: Crawled {crawl_result.pages_crawled} pages in {crawl_result.duration_seconds}s",
        )
        ...
        add_job_log(
            job_id_str,
            "INFO",
            f"Step 2: Classifying public emails from crawled pages...",
        )
        for page in crawl_result.pages:
            ...
            for em in page.discovered_emails:
                cat_em = classify_email(
                    em, page_type=page.page_type, source_url=page.url
                )
        ...
        add_job_log(
            job_id_str,
            "INFO",
            f"Step 3: Conducting public LinkedIn HR & Recruiter research...",
        )
        ...
        if self.enable_public_search:
            add_job_log(
                job_id_str,
                "INFO",
                f"Step 4: Querying public search engines & business indexes...",
            )
            try:
                search_results = await self.searcher.search_company_hr_info(
                    company_name=company_name, website_domain=website, max_results=6
                )
        ...
        add_job_log(
            job_id_str,
            "INFO",
            f"Step 5: Verifying emails and computing confidence score...",
        )
        (
            hr_email,
            recruitment_email,
        ...
        if all_discovered_profiles:
            ...
            best_profile = sorted(
                all_discovered_profiles, key=lambda x: x.confidence, reverse=True
            )[0]
        ...
        raw_audit_details["verification"] = {
            ...
            "all_profiles_discovered": [
                p.model_dump() for p in all_discovered_profiles
            ],
