...
PAGE_PATTERNS = {
    "careers": (
        re.compile(
            r"(careers?|job|jobs|hiring|openings|vacancies|work-with-us|join-us|join-our-team|opportunities)",
            re.I,
        ),
        10,
    ),
    "contact": (
        re.compile(
            r"(contact|contact-us|contactus|get-in-touch|reach-us|reach-out|touch)",
            re.I,
        ),
        9,
    ),
    "people": (
        re.compile(
            r"(people|our-people|talent|human-resources|hr|staffing|recruiters?)", re.I
        ),
        9,
    ),
    "team": (
        re.compile(
            r"(team|our-team|leadership|management|executives?|about-us|about|who-we-are)",
            re.I,
        ),
        8,
    ),
    "departments": (
        re.compile(r"(departments?|divisions?|offices?|locations?)", re.I),
        6,
    ),
...
def is_internal_url(base_domain: str, candidate_url: str) -> bool:
    ...
    try:
        parsed_base = urlparse(
            base_domain if "://" in base_domain else f"https://{base_domain}"
        )
